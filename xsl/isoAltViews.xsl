...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:xd="http://www.oxygenxml.com/ns/doc/xsl" exclude-result-prefixes="xd" version="2.0">
    ...
    <xsl:param name="docucompUrl" select="'http://www.ngdc.noaa.gov/docucomp'" />
    <xsl:param name="wafBaseUrl" select="'http://www.ngdc.noaa.gov/metadata/published/'" />
    ...
    <xsl:template name="isoAltViews">
        ...
        <xsl:choose>
            ...
            <xsl:when test="contains($xmlPath, '/iso/')">
                <xsl:choose>
                    ...
                    <xsl:otherwise>
                        <a title="View links to get the data from this dataset" href="/docucomp/page?xml={$xmlPath}&amp;view=getDataView&amp;header=none&amp;title=Get Data {$xmlPath}">Get Data</a>, </xsl:otherwise>
                </xsl:choose>
                <xsl:choose>
                    ...
                    <xsl:otherwise>
                        <a title="View as FAQ page" href="/docucomp/page?xml={$xmlPath}&amp;view=iso2faq/FAQ_ISO&amp;title=FAQ {$xmlPath}">FAQ</a>, </xsl:otherwise>
                </xsl:choose>
            </xsl:when>
        </xsl:choose>
        <xsl:choose>
            ...
            <xsl:otherwise>
                <a title="View as ISO indented text" href="/docucomp/page?xml={$xmlPath}&amp;view=xml2text/xml-to-text-ISO&amp;title=HTML {$xmlPath}">HTML, </a>
            </xsl:otherwise>
        </xsl:choose>
        <a title="View raw XML" href="http://www.ngdc.noaa.gov/metadata/published/{$xmlPath}">19139 XML</a>
        ...
        <xsl:choose>
            ...
            <xsl:when test="contains($xmlPath, '/iso/')">
                ...
                <xsl:choose>
                    ...
                    <xsl:otherwise>
                        <a title="View as ISO Rubric Report" href="/docucomp/page?xml={$xmlPath}&amp;view=isoRubricHTML&amp;title=ISO Rubric {$xmlPath}">Completeness, </a>
                    </xsl:otherwise>
                </xsl:choose>
                <xsl:choose>
                    ...
                    <xsl:otherwise>
                        <a title="View as DOI Rubric Report" href="/docucomp/page?xml={$xmlPath}&amp;view=doiRubricHTML&amp;title=DOI Rubric {$xmlPath}">DOI Readiness, </a>
                    </xsl:otherwise>
                </xsl:choose>
                <xsl:choose>
                    <xsl:when test="contains($xmlPath, '/iso/')">
                        <xsl:choose>
                            ...
                            <xsl:otherwise>
                                <a title="View links to get the data from this dataset" href="/docucomp/page?xml={$xmlPath}&amp;view=CSWRubricHTML&amp;title=CSW {$xmlPath}">CSW Readiness, </a>
                            </xsl:otherwise>
                        </xsl:choose>
                    </xsl:when>
                </xsl:choose>
        ...
        <xsl:choose>
            ...
            <xsl:otherwise>
                <a title="View as HTML components table" href="/docucomp/page?xml={$xmlPath}&amp;view=ISO19115Components-HTMLTable&amp;title=Components {$xmlPath}">Components</a>
            </xsl:otherwise>
        </xsl:choose>
